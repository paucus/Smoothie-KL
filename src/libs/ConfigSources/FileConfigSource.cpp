    ...
    while(!feof(lp)) {
        ...
        if(readLine(line, ln++, lp)) {
            ...
            if(cv->check_sums[0] == include_checksum) {
                ...
                // BEGIN MODIF config_file_not_string
                if(!file_exists(inc_file_name.c_str())) {
                // END MODIF config_file_not_string
                    ...
                    // BEGIN MODIF config_file_not_string
                    if(file_exists((path + inc_file_name).c_str())) inc_file_name = path + inc_file_name;
                    // then check root locations
                    // BEGIN MODIF second_config_file
                    else if(file_exists((("/" CONFIG_SD_MOUNT_DIR) + inc_file_name).c_str())) inc_file_name = ("/" CONFIG_SD_MOUNT_DIR) + inc_file_name;
                    // END MODIF second_config_file
                    else if(file_exists(("/local" + inc_file_name).c_str())) inc_file_name = "/local" + inc_file_name;
                    // END MODIF config_file_not_string
                }
                // BEGIN MODIF config_file_not_string
                if(file_exists(inc_file_name.c_str())) {
...
bool FileConfigSource::write( string setting, string value )
{
    ...
    // Open the config file ( find it if we haven't already found it )
    // BEGIN MODIF config_file_not_string
    FILE *lp = fopen(this->get_config_file(), "r+");
    // END MODIF config_file_not_string
    ...
    fclose(lp);
    // BEGIN MODIF config_file_not_string
    lp = fopen(this->get_config_file(), "a");
    // END MODIF config_file_not_string
    fputs("\n", lp);
    fputs(setting.c_str(), lp);
    fputs("         ", lp);
