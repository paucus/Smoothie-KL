...
Config::Config()
{
    ...
    if( file_exists("/" CONFIG_SD_MOUNT_DIR "/config") )
        fcs = new FileConfigSource("/" CONFIG_SD_MOUNT_DIR "/config", "sd");
    else if( file_exists("/" CONFIG_SD_MOUNT_DIR "/config.txt") )
        fcs = new FileConfigSource("/" CONFIG_SD_MOUNT_DIR "/config.txt", "sd");
    // END MODIF second_config_file
    // BEGIN MODIF config_factory
    if( fcs != NULL ) {
        this->config_sources.push_back( fcs );
        fcs = NULL;
    }
    ...
    ensure_file_exists("/" CONFIG_SD_MOUNT_DIR "/config-factory");
    if( file_exists("/" CONFIG_SD_MOUNT_DIR "/config-factory") )
        fcs = new FileConfigSource("/" CONFIG_SD_MOUNT_DIR "/config-factory", "sdfact");
    if( fcs != NULL ) {
	 this->config_sources.push_back( fcs );
        fcs = NULL;
    }
    // END MODIF config_factory
}
...
unsigned int Config::config_cache_size() const
{
	if (this->config_cache) {
		return this->config_cache->store_size();
	} else {
		return 0;
	}
}
...
HeavyWeightConfigValueProxy *Config::value(uint16_t check_sum_a, uint16_t check_sum_b, uint16_t check_sum_c )
...
// See get_checksum
// BEGIN MODIF cache_value_proxy
HeavyWeightConfigValueProxy *Config::value(uint16_t check_sums[])
// END MODIF cache_value_proxy
{
...
//    }
    ...
    return HeavyWeightConfigValueProxy::do_conf(result);
}
// END MODIF cache_value_proxy
