...
// BEGIN MODIF display
void Kernel::process_display_gcode_queue() {
    while (!display_gcodes.empty()) {
        SerialMessage* command = static_cast<SerialMessage*>(display_gcodes.front());
        display_gcodes.pop();
        call_event(ON_CONSOLE_LINE_RECEIVED, command);
        delete command;
    }
}

void Kernel::append_gcode_to_queue(const char* msg, StreamOutput* output) {
    SerialMessage* new_message = new SerialMessage();
    new_message->message = string(msg);
    new_message->stream = output;
    display_gcodes.push(new_message);
}

void Kernel::append_gcode_v_to_queue(const char* msgfmt, StreamOutput* output, ...) {
    char buff[100];
    va_list argptr;
    va_start(argptr, output);
    vsnprintf(buff, 100 - 1, msgfmt, argptr);
    va_end(argptr);
    append_gcode_to_queue(buff, output);
}
