...
class Config : public Module {
    public:
        ...
        unsigned int config_cache_size() const;
        // END MODIF config_cache_preallocation
        void config_cache_clear();
        void set_string( string setting , string value);

        // BEGIN MODIF cache_value_proxy
        HeavyWeightConfigValueProxy* value(uint16_t check_sum_a, uint16_t check_sum_b= 0, uint16_t check_sum_c= 0 );
        HeavyWeightConfigValueProxy* value(uint16_t check_sums[3] );
        // END MODIF cache_value_proxy
