...
Picaso_Serial_4DLib::Picaso_Serial_4DLib(SerialConsoleStream* virtualPort,
		PinName rst) {
	_virtualPort = virtualPort;
	_virtualPort->flush();

	reset_pin = new mbed::DigitalInOut(rst);
	reset_pin->output();
	reset_pin->write(1);
...
void Picaso_Serial_4DLib::reset_display() {
	reset_pin->write(0);
	delay(300);
	reset_pin->write(1);

	// Need to wait for display startup...
	delay(3000);
...
void Picaso_Serial_4DLib::WriteChars(const char * charsout) {
...
void Picaso_Serial_4DLib::SetThisBaudrate(int Newrate) {
	...
	case BAUD_115200:
		...
		break;
	case BAUD_128000:
		br = 133928; // previous was 133928 ;
		break;
	case BAUD_256000:
		br = 281250; // previous was 281250 ;
		break;
	}

	_virtualPort->set_baudrate(br);
}
...
// BEGIN MODIF const
word Picaso_Serial_4DLib::file_Erase(const char * Filename) {
// END MODIF const
...
// BEGIN MODIF const
word Picaso_Serial_4DLib::file_Exists(const char * Filename) {
// END MODIF const
...
word Picaso_Serial_4DLib::file_Index(word Handle, word HiSize, word LoSize,
		word Recordnum) {
...
// BEGIN MODIF thumbnails
word Picaso_Serial_4DLib::file_LoadImageControl(const char * Datname, const char * GCIName,
		word Mode) {
// END MODIF thumbnails
...
// BEGIN MODIF const
word Picaso_Serial_4DLib::file_Open(const char * Filename, char Mode) {
// END MODIF const
...
word Picaso_Serial_4DLib::file_ScreenCapture(word X, word Y, word Width,
		word Height, word Handle) {
...
void Picaso_Serial_4DLib::gfx_Button(word Up, word x, word y, word buttonColour,
		word txtColour, word font, word txtWidth, word txtHeight, char * text) {
...
void Picaso_Serial_4DLib::gfx_CircleFilled(word X, word Y, word Radius,
		word Color) {
...
void Picaso_Serial_4DLib::gfx_Ellipse(word X, word Y, word Xrad, word Yrad,
		word Color) {
...
void Picaso_Serial_4DLib::gfx_EllipseFilled(word X, word Y, word Xrad,
		word Yrad, word Color) {
...
void Picaso_Serial_4DLib::gfx_Line(word X1, word Y1, word X2, word Y2,
		word Color) {
...
word Picaso_Serial_4DLib::gfx_Orbit(word Angle, word Distance, word * Xdest,
		word * Ydest) {
...
void Picaso_Serial_4DLib::gfx_Panel(word Raised, word X, word Y, word Width,
		word Height, word Color) {
...
void Picaso_Serial_4DLib::gfx_Rectangle(word X1, word Y1, word X2, word Y2,
		word Color) {
...
void Picaso_Serial_4DLib::gfx_RectangleFilled(word X1, word Y1, word X2,
		word Y2, word Color) {
...
void Picaso_Serial_4DLib::gfx_ScreenCopyPaste(word Xs, word Ys, word Xd,
		word Yd, word Width, word Height) {
...
word Picaso_Serial_4DLib::gfx_Slider(word Mode, word X1, word Y1, word X2,
		word Y2, word Color, word Scale, word Value) {
...
void Picaso_Serial_4DLib::gfx_Triangle(word X1, word Y1, word X2, word Y2,
		word X3, word Y3, word Color) {
...
void Picaso_Serial_4DLib::gfx_TriangleFilled(word X1, word Y1, word X2, word Y2,
		word X3, word Y3, word Color) {
...
word Picaso_Serial_4DLib::img_ClearAttributes(word Handle, word Index,
		word Value) {
...
word Picaso_Serial_4DLib::img_SetAttributes(word Handle, word Index,
		word Value) {
...
word Picaso_Serial_4DLib::img_SetPosition(word Handle, word Index, word Xpos,
		word Ypos) {
...
word Picaso_Serial_4DLib::img_SetWord(word Handle, word Index, word Offset,
		word Word) {
...
// BEGIN MODIF const
word Picaso_Serial_4DLib::putstr(const char * InString) {
// END MODIF const
...
word Picaso_Serial_4DLib::putstr(const char * InString, word from, word to) {
	_virtualPort->print((char) (F_putstr >> 8));
	_virtualPort->print((char) (F_putstr));
	WriteBytes((char*)&(InString[from]), to - from);
	_virtualPort->write((unsigned char)'\0');
	return GetAckResp();
}
// END MODIF wordwrap
...
void Picaso_Serial_4DLib::touch_DetectRegion(word X1, word Y1, word X2,
		word Y2) {
...
word Picaso_Serial_4DLib::file_CallFunction(word Handle, word ArgCount,
t4DWordArray Args)
{
...
void Picaso_Serial_4DLib::blitComtoDisplay(word X, word Y, word Width,
		word Height, t4DByteArray Pixels)
{
