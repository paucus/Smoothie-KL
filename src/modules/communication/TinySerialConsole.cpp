...
TinySerialConsole::TinySerialConsole( PinName rx_pin, PinName tx_pin, int baud_rate ){
    this->serial = new mbed::Serial( rx_pin, tx_pin );
    this->serial->baud(baud_rate);
}

// Called when the module has just been loaded
void TinySerialConsole::on_module_loaded() {
    // We want to be called every time a new char is received
    this->serial->attach(this, &TinySerialConsole::on_serial_char_received, mbed::Serial::RxIrq);
...
void TinySerialConsole::on_serial_char_received(){
...
void TinySerialConsole::on_main_loop(void * argument){
    if( this->has_char('\n') ){
        string received;
        received.reserve(20);
        while(1){
...
int TinySerialConsole::puts(const char* s)
{
    return fwrite(s, strlen(s), 1, (FILE*)(*this->serial));
}

int TinySerialConsole::_putc(int c)
{
    return this->serial->putc(c);
}

int TinySerialConsole::_getc()
{
    return this->serial->getc();
}

// Does the queue have a given char ?
bool TinySerialConsole::has_char(char letter){
