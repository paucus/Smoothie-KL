    ...
    if ( first_char == 'G' || first_char == 'M' || first_char == 'T' || first_char == 'N' ) {
        ...
        if( cs == 0x00 && ln == nextline ) {
            ...
            while(possible_command.size() > 0) {
                ...
                // BEGIN MODIF only one gcode per line
                size_t nextcmd = string::npos; //possible_command.find_first_of("GM", 2);
                // END MODIF only one gcode per line
                    ...
                    if(gcode->has_m) {
                        switch (gcode->m) {
                            case 28: // start upload command
                                ...
                                this->upload_filename = ("/" PUBLIC_SD_MOUNT_DIR "/") + single_command.substr(4); // rest of line is filename
                            ...
                            case 510: // M510 save some critical volatile settings to config-priv
                                // replace stream with one that writes to config-priv file
                                gcode->stream = new FileStream(THEKERNEL->config_priv_filename());
                                // dispatch the M500 here so we can free up the stream when done
                                THEKERNEL->call_event(ON_GCODE_RECEIVED, gcode );
                                delete gcode->stream;
                                delete gcode;
                                new_message.stream->printf("Settings Stored to %s\r\nok\r\n", THEKERNEL->config_priv_filename());
                                continue;

                            case 513: { // M513 display live settings and indicates if there is an override file
                                FILE *fd = fopen(THEKERNEL->config_priv_filename(), "r");
                                if(fd != NULL) {
                                    fclose(fd);
                                    new_message.stream->printf("; config private override present: %s\n",  THEKERNEL->config_priv_filename());

                                } else {
                                    new_message.stream->printf("; No config private override\n");
                                }
                                break; // fall through to process by modules
                            }
