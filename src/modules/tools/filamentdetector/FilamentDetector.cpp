...
void FilamentDetector::on_gcode_received(void *argument)
{
    ...
    if (gcode->has_m) {
        ...
        // BEGIN MODIF filament
        } else if (gcode->m == 743 || gcode->m == 744) { // Queue command. Enable/disable filament detector (blocking)
            THEKERNEL->conveyor->append_gcode(gcode);
            // ensure that no subsequent gcodes get executed along with it
            THEKERNEL->conveyor->queue_head_block();
        } else if (gcode->m == 745) {    // M745: Filament present? (in this case, "moving")
            gcode->stream->printf("filament_moving(F): %d\r\n", filament_out_alarm?0:1);
        // END MODIF filament
        }
    }
}
// BEGIN MODIF filament
void FilamentDetector::on_gcode_execute(void *argument)
{
    Gcode* gcode = static_cast<Gcode*>(argument);
    if (gcode->has_m && gcode->m == 743) { // enable filament detector (blocking)
        this->pulses= 0;
        e_last_moved=  get_emove();
        this->active = true;
    } else if (gcode->has_m && gcode->m == 744) { // disable filament detector (blocking)
        this->active = false;
        e_last_moved= get_emove();
