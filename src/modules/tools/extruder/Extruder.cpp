...
// BEGIN MODIF extruder_multiplier cold_extrusion_prevention
#include "modules/tools/temperaturecontrol/TemperatureControlPublicAccess.h"
#include "PublicData.h"
// END MODIF extruder_multiplier cold_extrusion_prevention
...
// BEGIN MODIF cold_extrusion_prevention
#define cold_extrusion_prevention_feature_enabled_checksum  CHECKSUM("cold_extrusion_prevention_feature_enabled")
#define cold_extrusion_temperature_checksum         CHECKSUM("cold_extrusion_temperature")

#include "Uptime.h"
// 60 seconds between two cold extrusion notifications
#define MAX_INTERVAL_BEFORE_NEXT_REPORT 30
// END MODIF cold_extrusion_prevention
...
void Extruder::on_config_reload(void *argument)
{
    if( this->single_config ) {
        ...
        // BEGIN MODIF locomotora
        // Let the extruder_steps_per_mm_checksum parameter override extruder.id.steps_per_mm.
        float steps_per_mm_old_style = THEKERNEL->config->value(extruder_steps_per_mm_checksum)->by_default(-1)->as_number();
        if (this->identifier == CHECKSUM("hotend") && steps_per_mm_old_style > 0) {
            this->steps_per_millimeter = steps_per_mm_old_style;
        } else {
    ...
    // BEGIN MODIF cold_extrusion_prevention
    this->cold_extrusion_prevention_feature_enabled = THEKERNEL->config->value(extruder_checksum, this->identifier, cold_extrusion_prevention_feature_enabled_checksum)->by_default(true)->as_bool();
    this->cold_extrusion_prevention_event_enabled = true;	// this is a flag only
    this->cold_extrusion_temperature = THEKERNEL->config->value(extruder_checksum, this->identifier, cold_extrusion_temperature_checksum)->by_default(170)->as_number();
    this->cold_extrusion = false;
    this->cold_extrusion_last_report = uptime() - MAX_INTERVAL_BEFORE_NEXT_REPORT;
    // END MODIF cold_extrusion_prevention
}
...
void Extruder::on_get_public_data(void *argument)
{
    ...
    if(this->enabled) {
        ...
        if (pdr->second_element_is(target_position_checksum)) {
            (*static_cast<float*>(pdr->get_data_ptr())) = this->target_position;
        } else if (pdr->second_element_is(absolute_mode_checksum)) {
            (*static_cast<bool*>(pdr->get_data_ptr())) = this->absolute_mode;
        // BEGIN MODIF extruder_multiplier
        } else if (pdr->second_element_is(extruder_multiplier_checksum)) {
            (*static_cast<float*>(pdr->get_data_ptr())) = this->extruder_multiplier;
...
void Extruder::on_gcode_received(void *argument)
{
    ...
    if (gcode->has_m) {
        ...
        // BEGIN MODIF cold_extrusion_prevention
        } else if (gcode->m == 302 || gcode->m == 702) {
            THEKERNEL->conveyor->append_gcode(gcode);
        // END MODIF cold_extrusion_prevention
        }
    ...
    if( gcode->has_m ) {
        switch(gcode->m) {
            ...
            case 84:
                ...
                break;
        // BEGIN MODIF cold_extrusion_prevention
            case 302: {	// M302: Allow cold extrudes. This tells the printer to allow movement of the extruder motor, when the hotend is not at printing temperature. To allow or deny cold extrudes/retracts, run either "M302 P1" or "M302 P0".
                this->cold_extrusion_prevention_feature_enabled = gcode->has_letter('P')?(gcode->get_value('P') != 0.0):false;
                this->cold_extrusion_last_report = uptime() - MAX_INTERVAL_BEFORE_NEXT_REPORT;
                break;
                }
            case 702: {	// M702: Disable Cold extrusion prevention event notification. This will make the printer prevent the extrusion itself, but won't show a message or stop the printing.
                this->cold_extrusion_prevention_event_enabled = gcode->has_letter('P')?(gcode->get_value('P') != 0.0):false;
                this->cold_extrusion_last_report = uptime() - MAX_INTERVAL_BEFORE_NEXT_REPORT;
                break;
                }
        // END MODIF cold_extrusion_prevention
    ...
    if( gcode->has_g && this->enabled ) {
        ...
        if( gcode->g == 92 ) {
            ...
            if( gcode->has_letter('E' )) {
                ...
                // BEGIN MODIF extruder_multiplier cold_extrusion_prevention
                this->cold_extrusion = get_current_hotend_temp() < cold_extrusion_temperature;
                if (cold_extrusion_prevention_feature_enabled && cold_extrusion) {
                    // set ratio = 0 to avoid extruding cold plastic
                    if (this->mode == SOLO)
...
// BEGIN MODIF cold_extrusion_prevention
float Extruder::get_current_hotend_temp(){
    // get the temperature control instance with this name
    struct pad_temperature temp;
    bool ok = PublicData::get_value( temperature_control_checksum, current_temperature_checksum, hotend_checksum, &temp );
...
// BEGIN MODIF cold_extrusion_prevention
void Extruder::on_idle( void *argument ) {
    if (cold_extrusion && cold_extrusion_prevention_feature_enabled && cold_extrusion_prevention_event_enabled && cold_extrusion_last_report+MAX_INTERVAL_BEFORE_NEXT_REPORT < uptime()) {
        cold_extrusion_last_report = uptime();
        THEKERNEL->call_event(ON_COLD_EXTRUSION, this);
        // after calling the event, clean the cold extrusion parameter. Otherwise it would keep on
        // showing until another extrusion is performed.
        cold_extrusion = false;
    }
}
// END MODIF cold_extrusion_prevention
// BEGIN MODIF client_print
void Extruder::on_print_status_change(void * args) {
    // Whenever a print is started or finished, reset the extruder multiplier
    print_status_change_t e = *(static_cast<print_status_change_t*>(args));
    if (e.event == pe_begin || e.event == pe_end) {
        // reset the extruder multiplier
        this->extruder_multiplier = 1.0;
    }
}
// END MODIF client_print
