...
{
    // BEGIN MODIF pid_autotune_screen
    pid_autotune_event_t pid_autotune_event;
    pid_autotune_event.pool_index = temp_control->pool_index;
    pid_autotune_event.name_checksum = temp_control->name_checksum;
    pid_autotune_event.event = pid_autotune_begin;
//    pid_autotune_event.temp_control = temp_control;
//    pid_autotune_event.ncycles = ncycles;	// target cycles
    pid_autotune_event.temp = target;	// target temp
    THEKERNEL->call_event(ON_PID_AUTOTUNE_EVENT, &pid_autotune_event);
    // END MODIF pid_autotune_screen
    noiseBand = 0.5;
...
void PID_Autotuner::abort()
{
    ...
    pid_autotune_event.pool_index = temp_control->pool_index;
    pid_autotune_event.name_checksum = temp_control->name_checksum;
    pid_autotune_event.event = pid_autotune_abort;
//    pid_autotune_event.temp_control = temp_control;
//    pid_autotune_event.ncycles = peakCount;	// last peak
    pid_autotune_event.temp = temp_control->get_temperature();	// last temperature
    THEKERNEL->call_event(ON_PID_AUTOTUNE_EVENT, &pid_autotune_event);
    // END MODIF pid_autotune_screen
...
void PID_Autotuner::finishUp()
{
    ...
    pid_autotune_event.pool_index = temp_control->pool_index;
    pid_autotune_event.name_checksum = temp_control->name_checksum;
    pid_autotune_event.event = pid_autotune_end;
//    pid_autotune_event.temp_control = temp_control;
//    pid_autotune_event.ncycles = peakCount;	// last peak
    pid_autotune_event.temp = temp_control->get_temperature();	// last temperature
    THEKERNEL->call_event(ON_PID_AUTOTUNE_EVENT, &pid_autotune_event);
