        ...
        switch( gcode->m ) {
            ...
            case 374:{
                ...
                if(this->saveGrid(gridname)) {
                    ...
                    gcode->stream->printf("Grid saved: Filename: /" CONFIG_SD_MOUNT_DIR "/Zgrid.%s\n",gridname);
                    // END MODIF second_config_file
                }
                else {
                    // BEGIN MODIF second_config_file
                    gcode->stream->printf("Error: Grid not saved: Filename: /" CONFIG_SD_MOUNT_DIR "/Zgrid.%s\n",gridname);
                    // END MODIF second_config_file
                }
            }
            return true;
            ...
            case 375:{ // Load grid values
                ...
                if(this->loadGrid(gridname)) {
                    this->setAdjustFunction(true); // Enable leveling code
                    // BEGIN MODIF second_config_file
                    gcode->stream->printf("Grid loaded: /" CONFIG_SD_MOUNT_DIR "/Zgrid.%s\n",gridname);
                    // END MODIF second_config_file
                }
                else {
                    // BEGIN MODIF second_config_file
                    gcode->stream->printf("Error: Grid not loaded: /" CONFIG_SD_MOUNT_DIR "/Zgrid.%s\n",gridname);
                    // END MODIF second_config_file
                }
            }
            return true;
...
{
    // BEGIN MODIF second_config_file
    args = "/" CONFIG_SD_MOUNT_DIR "/Zgrid." + args;
    // END MODIF second_config_file
