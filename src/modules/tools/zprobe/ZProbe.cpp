...
// BEGIN MODIF multi_point_leveling_strategy
#include "MultiPointStrategy.h"
// END MODIF multi_point_leveling_strategy
        ...
        if( THEKERNEL->config->value(leveling_strategy_checksum, cs, enable_checksum )->as_bool() ){
            ...
            switch(cs) {
                ...
                case three_point_leveling_strategy_checksum:
                    ...
                    break;
// BEGIN MODIF multi_point_leveling_strategy
                case multi_point_leveling_strategy_checksum:
                    // NOTE this strategy is mutually exclusive with the delta calibration strategy
                    this->strategies.push_back(new MultiPointStrategy(this));
                    found= true;
                    break;
// END MODIF multi_point_leveling_strategy
...
void ZProbe::on_get_public_data(void *argument) {
    ...
    if(pdr->second_element_is(delta_mm_from_bed_checksum)) {
        (*static_cast<float*>(pdr->get_data_ptr())) = delta_mm_from_bed();
        pdr->set_taken();
    }
}
static float get_current_bed_temp(){
    // get the temperature control instance with this name
    struct pad_temperature temp;
    bool ok = PublicData::get_value( temperature_control_checksum, current_temperature_checksum, bed_checksum, &temp );
...
int ZProbe::delta_steps_from_bed() {
    // Correct the measured distance according to the temperature correction
    float temp = get_current_bed_temp();
    float delta_mm = temp_correction.get_value(temp);
    return delta_mm * Z_STEPS_PER_MM;
}
float ZProbe::delta_mm_from_bed() {
    // Correct the measured distance according to the temperature correction
    float temp = get_current_bed_temp();
    return temp_correction.get_value(temp);
}
// END MODIF correct_nozzle_distance_to_bed
...
bool ZProbe::doProbeAt(int &steps, float x, float y)
{
    ...
    return_probe(s);
    // BEGIN MODIF correct_nozzle_distance_to_bed
    // Now return the measured distance corrected by temperature.
    steps = s + delta_steps_from_bed();
    // END MODIF correct_nozzle_distance_to_bed
...
// BEGIN MODIF autolevel
static bool is_non_leveling_home(Gcode* gcode) {
    // This condition is the same of Endstops.cpp
...
void ZProbe::on_gcode_received(void *argument)
{
    ...
    // BEGIN MODIF autolevel
    if( gcode->has_g && gcode->g >= 28 && gcode->g <= 32) { // Let the autolevel strategies try to handle G28 if necessary.
        if (gcode->g == 28  && (!autolevel_on_home || is_non_leveling_home(gcode)) ) {
            return;
        }
        // END MODIF autolevel
...
//        }
        ...
        if( gcode->g == 30 ) { // simple Z probe
            ...
            if(probe_result) {
                // BEGIN MODIF correct_nozzle_distance_to_bed
                // Now return the measured distance corrected by temperature.
                int corrected_steps = steps + delta_steps_from_bed();
                gcode->stream->printf("Z:%1.4f C:%d\n", corrected_steps / Z_STEPS_PER_MM, corrected_steps);
                // END MODIF correct_nozzle_distance_to_bed
        ...
        switch (gcode->m) {
            ...
            case 670:
                ...
                break;
            // BEGIN MODIF correct_nozzle_distance_to_bed
            case 737:     // M737 Set Z Probe zprobe_length
                if (gcode->has_letter('S') && gcode->has_letter('Z')) {
                    float temp = gcode->get_value('S');
                    float length = gcode->get_value('Z');
                    temp_correction.set_point(temp, length);
                } else if (gcode->has_letter('C') && gcode->get_value('C') != 0){
                    if (gcode->has_letter('S')) {
                        float temp = gcode->get_value('S');
                        temp_correction.remove_point(temp);
                    } else {
                        temp_correction.clear();
                        gcode->stream->printf("Temp Values reset\r\n");
                    }
                } else if (gcode->has_letter('S') && !gcode->has_letter('Z')) {
                    float temp = gcode->get_value('S');
                    gcode->stream->printf("Length at %f=%f\r\n", temp, temp_correction.get_value(temp));
                }

                gcode->stream->printf("zprobe_length_correction:\r\n");
                for (std::set<std::pair<float, float>, bool(*)(std::pair<float, float>, std::pair<float, float>)>::iterator it = temp_correction.points.begin(); it != temp_correction.points.end(); it++) {
                    std::pair<float, float> p = *it;
                    gcode->stream->printf("%.2f=>%.4fmm\r\n", p.first, p.second);
                }
                gcode->stream->printf("\r\n");
            break;
            case 510: // save settings
            case 513: // print settings
                gcode->stream->printf(";Z probe length (mm):\r\nM737 C1\r\n");
                for (std::set<std::pair<float, float>, bool(*)(std::pair<float, float>, std::pair<float, float>)>::iterator it = temp_correction.points.begin(); it != temp_correction.points.end(); it++) {
                    std::pair<float, float> p = *it;
                    gcode->stream->printf("M737 S%f Z%f\r\n", p.first, p.second);
                }
            break;
            // END MODIF correct_nozzle_distance_to_bed
...
void ZProbe::coordinated_move(float x, float y, float z, float feedrate, bool relative)
{
	...
	char buffy[11] = "";
	char buffz[11] = "";
	if(!isnan(x)) snprintf(buffx, sizeof(buffx), " X%1.3f", x);
	if(!isnan(y)) snprintf(buffy, sizeof(buffy), " Y%1.3f", y);
	if(!isnan(z)) snprintf(buffz, sizeof(buffz), " Z%1.3f", z);
	send_robot_gcode_wait_v(relative, "G0 F%1.1f%s%s%s", &(StreamOutput::NullStream), feedrate * 60, buffx, buffy, buffz);
}
// END MODIF autolevel
