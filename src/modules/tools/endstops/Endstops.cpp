...
#define Z_AXIS 2
// BEGIN MODIF fsr
#define GAMMA_MIN_PIN_INDEX 2
#define GAMMA_MAX_PIN_INDEX 5
// END MODIF fsr
...
#define safe_homing_checksum                    CHECKSUM("safe_homing")
#define x_safe_homing_position_checksum         CHECKSUM("x_safe_homing_position")
#define y_safe_homing_position_checksum         CHECKSUM("y_safe_homing_position")
//#define z_safe_homing_position_checksum         CHECKSUM("z_safe_homing_position")
// END MODIF safe_homing
// BEGIN MODIF autolevel
#define autolevel_on_home_checksum              CHECKSUM("autolevel_on_home")
#define z_safe_homing_position_checksum         CHECKSUM("z_safe_homing_position")
// END MODIF autolevel
// BEGIN MODIF limit_endstops
#define physical_boundary_endstop_gamma_enabled_checksum         CHECKSUM("physical_boundary_endstop_gamma_enabled")
...
Endstops::Endstops()
{
    ...
    // BEGIN MODIF safe_homing
    for(int i=0;i<3;i++)
        axis_position_known[i] = false;
    autolevel_on_home = true;
    end_safe_homing_at_center_of_heatbed = 0;
    // END MODIF safe_homing
}
...
void Endstops::on_config_reload(void *argument)
{
    ...
    // BEGIN MODIF limit_endstops
    this->physical_boundary_endstop_gamma_enabled = THEKERNEL->config->value(physical_boundary_endstop_gamma_enabled_checksum)->by_default(false)->as_bool();
    // END MODIF limit_endstops
    ...
    // BEGIN MODIF limit_endstops
    } else if (this->physical_boundary_endstop_gamma_enabled) {
        register_for_event(ON_IDLE);
    // END MODIF limit_endstops
    }
    ...
    // BEGIN MODIF safe_homing
    safe_homing_enabled = THEKERNEL->config->value(safe_homing_checksum   )->by_default(true)->as_bool();
    safe_homing_position[X_AXIS] = THEKERNEL->config->value(x_safe_homing_position_checksum   )->by_default(178)->as_number();
    safe_homing_position[Y_AXIS] = THEKERNEL->config->value(y_safe_homing_position_checksum   )->by_default(100)->as_number();
    // safe_homing_position[Z_AXIS] is the lift Z distance. It is used in MultiPointStrategy.
    end_safe_homing_at_center_of_heatbed = THEKERNEL->config->value(end_safe_homing_at_center_of_heatbed_checksum   )->by_default( false )->as_bool();
    // END MODIF safe_homing
    // BEGIN MODIF autolevel
    autolevel_on_home = THEKERNEL->config->value(autolevel_on_home_checksum)->by_default(true)->as_bool();
...
void Endstops::on_idle(void *argument)
{
    ...
    // BEGIN MODIF limit_endstops
    if (this->physical_boundary_endstop_gamma_enabled) {
        if(STEPPER[Z_AXIS]->is_moving() && !STEPPER[Z_AXIS]->get_direction()) { // !direction = moving up
            // check max endstop
            int n = Z_AXIS + 3;
            if (this->pins[n].get()) {
                // MAX_POSITION DETECTED!!!!!!!!!! SET CURRENT POSITION AND LIMIT!!!!!
                StepperMotor* s = STEPPER[Z_AXIS];
                if (s->get_current_position_steps() < s->get_max_position_steps()) {
                    s->set_max_position_steps(s->get_current_position_steps());
                }
            }
        }
    } else {
...
// BEGIN MODIF correct_nozzle_distance_to_bed
void Endstops::move_to_offset_z()
{
    this->status = MOVE_TO_ORIGIN;
    float rate= this->fast_rates[Z_AXIS]*60.0F;
    char buf[32];
    snprintf(buf, sizeof(buf), "G0 Z%1.4f F%1.4f", this->homing_position[Z_AXIS] + home_offset[Z_AXIS], rate);
    Gcode gc(buf, &(StreamOutput::NullStream));
    THEKERNEL->robot->on_gcode_received(&gc); // send to robot directly
...
void Endstops::corexy_home(int home_axis, bool dirx, bool diry, float fast_rate, float slow_rate, unsigned int retract_steps)
{
    ...
    // BEGIN MODIF autolevel
    float slope = 0.0;
    // FIXME Considera los signos!!!!!!!!!!!!!!!
    if (THEKERNEL->robot->slopeTransform) {
        struct Robot::slope_pt p(home_axis == X_AXIS?1.0:0.00, home_axis == Y_AXIS?1.0:0.0);
        THEKERNEL->robot->slopeTransform(p, &slope);
    }
    bool dirz = ((slope > 0) == this->home_direction[Z_AXIS]) == this->home_direction[home_axis];
    slope = fabs(slope);
    // END MODIF autolevel
    ...
    // BEGIN MODIF autolevel
    this->feed_rate[Z_AXIS]= fast_rate * slope;
    ...
    // BEGIN MODIF autolevel
    // Stop Z motor if still running
    if (STEPPER[Z_AXIS]->is_moving()) STEPPER[Z_AXIS]->move(0, 0);
    float secant_slope = sqrt(1 + pow(slope, 2));       // 1 / cosine
    float sine_slope = slope / secant_slope;  // sine = tg * cos = tg / secant = slope / secant
    // END MODIF autolevel

    // Move back a small distance
    this->status = MOVING_BACK;
    this->feed_rate[X_AXIS]= slow_rate;
    STEPPER[X_AXIS]->move(!dirx, retract_steps / secant_slope, 0);
    this->feed_rate[Y_AXIS]= slow_rate;
    STEPPER[Y_AXIS]->move(!diry, retract_steps / secant_slope, 0);
    // BEGIN MODIF autolevel
    this->feed_rate[Z_AXIS]= slow_rate * slope;
    unsigned int steps = round(retract_steps * sine_slope * STEPS_PER_MM(Z_AXIS) / STEPS_PER_MM(home_axis));
    STEPPER[Z_AXIS]->move(!dirz, steps, 0);
    // END MODIF autolevel
    ...
    // BEGIN MODIF autolevel
    if (STEPPER[Z_AXIS]->is_moving()) STEPPER[Z_AXIS]->move(0, 0); // Stop Z motor if still running
    // END MODIF autolevel
...
void Endstops::do_homing_corexy(char axes_to_move)
{
    ...
    if((axes_to_move & 0x03) == 0x03) { // both X and Y need Homing
        ...
        // FIXME Considera los signos!!!!!!!!!!!!!!!
        if (THEKERNEL->robot->slopeTransform) {
            struct Robot::slope_pt p(0.70711, 0.70711);
            THEKERNEL->robot->slopeTransform(p, &slope);
        }
        bool dirz = ((slope > 0) == this->home_direction[Z_AXIS]) == this->home_direction[X_AXIS];// FIXME Considera los signos de direccion de los ejes!!!!!!!!!!!!!!!
        slope = fabs(slope);
        this->feed_rate[Z_AXIS]= this->fast_rates[motor] * slope;
            STEPPER[Z_AXIS]->move(dirz, 10000000, 0);
        // END MODIF autolevel
...
// BEGIN MODIF correct_nozzle_distance_to_bed
static float get_delta_mm_from_bed() {
    // get the temperature control instance with this name
    float returned_data;
    bool ok = PublicData::get_value( zprobe_checksum, delta_mm_from_bed_checksum, &returned_data );

    if (ok) {
        return returned_data;
    } else {
        return 0; // zprobe disabled
    }
}
// END MODIF correct_nozzle_distance_to_bed
...
void Endstops::on_gcode_received(void *argument)
{
    ...
    if ( gcode->has_g) {
        // BEGIN MODIF safe_homing
        // Only home this way if a home X or Y was requested but not Z or all, or when a home without autolevel is requested.
        if ( gcode->g == 28 ) {
            // This condition is the same of ZProbe.cpp
            bool non_leveling_home = (
                    (gcode->has_letter('L') && !gcode->get_uint('L')) || // Home without autolevel requested
                    (!gcode->has_letter('Z') && (gcode->has_letter('X') || gcode->has_letter('Y') ) ) // Home X/Y but not Z/all
                );
            if (autolevel_on_home && !non_leveling_home) {
                return;
            }
            if (!autolevel_on_home) {
...
//            bool home_all = this->is_delta || this->is_scara || !( gcode->has_letter('X') || gcode->has_letter('Y') || gcode->has_letter('Z') );
            bool home_all = this->is_delta || this->is_scara || !( gcode->has_letter('X') || gcode->has_letter('Y') || gcode->has_letter('Z') ) || (gcode->has_letter('X') && gcode->has_letter('Y') && gcode->has_letter('Z') );
            // END MODIF safe_homing
            ...
            // BEGIN MODIF correct_nozzle_distance_to_bed
            // If a zprobe with temperature Z correction is present, adapt the distance.
            float delta_mm_from_bed = get_delta_mm_from_bed();
            // END MODIF correct_nozzle_distance_to_bed
            // BEGIN MODIF safe_homing
            if (safe_homing_enabled) {
                bool is_homing_z = axes_to_move & (1 << Z_AXIS);

                // If we are safe homing, we need to know X and Y first. If not known, also home X and Y.
                if (is_homing_z && !(this->axis_position_known[X_AXIS] && this->axis_position_known[Y_AXIS]))
                    axes_to_move = (1<< X_AXIS) | (1<< Y_AXIS) | (1<< Z_AXIS);

                if (is_homing_z) {
                    // First lift Z a bit
                	send_robot_gcode_wait_v(true, "G0 Z%1.2f F%1.2f", &(StreamOutput::NullStream), this->lift_z_distance, this->fast_rates[Z_AXIS]*60.0F);
                }
                // Then we need to home X and Y but not Z.
                // Remove the Z_AXIS home
                char axes_to_move_except_z = axes_to_move & ~(1<< Z_AXIS);

                if (axes_to_move_except_z) {
                    home(axes_to_move_except_z);
                    // Zero the ax(i/e)s position, add in the home offset
                    if ( axes_to_move & (1 << X_AXIS) ) THEKERNEL->robot->reset_axis_position(this->homing_position[X_AXIS] + this->home_offset[X_AXIS], X_AXIS);
                    if ( axes_to_move & (1 << Y_AXIS) ) THEKERNEL->robot->reset_axis_position(this->homing_position[Y_AXIS] + this->home_offset[Y_AXIS], Y_AXIS);
                }

                if (is_homing_z) {
                	send_robot_gcode_wait_v(false, "G0 X%1.2f Y%1.2f F%1.2f", &(StreamOutput::NullStream), this->safe_homing_position[X_AXIS], this->safe_homing_position[Y_AXIS], this->fast_rates[X_AXIS]*60.0F);

                    home(1 << Z_AXIS);
                    // BEGIN MODIF correct_nozzle_distance_to_bed
                    THEKERNEL->robot->reset_axis_position(this->homing_position[Z_AXIS] + this->home_offset[Z_AXIS] + delta_mm_from_bed, Z_AXIS);
                    move_to_offset_z();    // finish the move that was started
                    // END MODIF correct_nozzle_distance_to_bed

                    // Go home
                    if (!end_safe_homing_at_center_of_heatbed) {
                        send_robot_gcode_wait_v(false, "G0 X0 Y0 Z0 F%1.2f", &(StreamOutput::NullStream), this->fast_rates[X_AXIS]*60.0F);
                    }
                }

                this->axis_position_known[X_AXIS] |= axes_to_move & (1<<X_AXIS);
                this->axis_position_known[Y_AXIS] |= axes_to_move & (1<<Y_AXIS);
                this->axis_position_known[Z_AXIS] |= axes_to_move & (1<<Z_AXIS);
                ...
                for (char c = 'X'; c <= 'Z'; c++)
                	gcode->stream->printf(" %c:%f", c, robot_position[c-'X']);
                gcode->stream->printf("\r\n");
            } else {
            // END MODIF safe_homing
                ...
                if(home_all) {
                    ...
                    float ideal_position[3] = {
                        ...
                        // BEGIN MODIF correct_nozzle_distance_to_bed
                        this->homing_position[Z_AXIS] + this->home_offset[Z_AXIS] + delta_mm_from_bed
                        // END MODIF correct_nozzle_distance_to_bed
                    };
                    ...
                    for ( int c = X_AXIS; c <= Z_AXIS; c++ ) {
                        if ( (axes_to_move >> c)  & 1 ) {
                            // BEGIN MODIF correct_nozzle_distance_to_bed
                            THEKERNEL->robot->reset_axis_position(this->homing_position[c] + this->home_offset[c] + (c == Z_AXIS)?delta_mm_from_bed:0, c);
                            // END MODIF correct_nozzle_distance_to_bed
                        }
                    }
		...
		// BEGIN MODIF autolevel
		} else if (gcode->g == 36) {    // G36 make single measure
			THEKERNEL->conveyor->wait_for_empty_queue();
			THEKERNEL->stepper->turn_enable_pins_on();

			bool dirz = home_direction[Z_AXIS];

			this->feed_rate[GAMMA_STEPPER]= this->slow_rates[Z_AXIS];
			STEPPER[GAMMA_STEPPER]->set_speed(0); // need to allow for more ground covered when moving diagonally
			STEPPER[GAMMA_STEPPER]->move(dirz, 10000000);
			this->status = MOVING_TO_ENDSTOP_FAST;

			while ( !(dirz?this->pins[GAMMA_MIN_PIN_INDEX].get():this->pins[GAMMA_MAX_PIN_INDEX].get()) ) {
				//THEKERNEL->call_event(ON_IDLE);
			}
			this->status = NOT_HOMING;

			uint32_t steps_moved = STEPPER[GAMMA_STEPPER]->get_stepped();
			if (gcode->has_letter('L') && gcode->get_value('L') != 0.0) {
			   float p[3];
			   THEKERNEL->robot->get_axis_position(p);
				gcode->stream->printf("Info:XYZ=%f %f %f\r\n", p[0], p[1], (float)(steps_moved/STEPS_PER_MM(GAMMA_STEPPER)));
			} else {
				gcode->stream->printf("Info: Steps: %lu\r\n", steps_moved);
			}

			STEPPER[GAMMA_STEPPER]->set_speed(0);
			this->feed_rate[GAMMA_STEPPER]= this->slow_rates[Z_AXIS];

			STEPPER[GAMMA_STEPPER]->move(!dirz, steps_moved);    // return to original position
			this->status = MOVING_TO_ENDSTOP_FAST;
			while ( STEPPER[GAMMA_STEPPER]->is_moving() ) {
				//THEKERNEL->call_event(ON_IDLE);
			}
			this->status = NOT_HOMING;
            // END MODIF autolevel
        ...
        switch (gcode->m) {
            ...
            // BEGIN MODIF safe_homing
            case 740: {    // M740 End safe homing at center of heatbed
                if (gcode->has_letter('C')) {
                    this->end_safe_homing_at_center_of_heatbed = (gcode->get_value('C') != 0);
                }
                gcode->stream->printf("End safe homing at center of heatbed: %d\r\n", end_safe_homing_at_center_of_heatbed);
            }
            break;
            // END MODIF safe_homing
...
void Endstops::on_get_public_data(void* argument){
    ...
    if(pdr->starts_with(end_safe_homing_at_center_of_heatbed_checksum)) {
        *static_cast<bool*>(pdr->get_data_ptr()) = this->end_safe_homing_at_center_of_heatbed;
        pdr->set_taken();
    }
    ...
    if(pdr->second_element_is(trim_checksum)) {
        ...
        pdr->set_taken();
    // BEGIN MODIF show_print
    }else if(pdr->second_element_is(axis_position_known_checksum)) {
        pdr->set_data_ptr(&this->axis_position_known);
        pdr->set_taken();
    // END MODIF show_print
    ...
    // BEGIN MODIF autolevel
    }else if(pdr->second_element_is(fast_rates_checksum)) {
        pdr->set_data_ptr(this->fast_rates);
        pdr->set_taken();
    }else if(pdr->second_element_is(slow_rates_checksum)) {
        pdr->set_data_ptr(this->slow_rates);
        pdr->set_taken();
    // END MODIF autolevel
    }
}
