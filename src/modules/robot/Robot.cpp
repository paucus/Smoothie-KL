...
// BEGIN MODIF move_ony_if_axis_position_known
#include "EndstopsPublicAccess.h"
// END MODIF move_ony_if_axis_position_known
...
// BEGIN MODIF boundaries
#define printing_area_boundary_max_x_checksum CHECKSUM("printing_area_boundary_max_x")
#define printing_area_boundary_max_y_checksum CHECKSUM("printing_area_boundary_max_y")
#define printing_area_boundary_max_z_checksum CHECKSUM("printing_area_boundary_max_z")
#define printing_area_boundary_min_x_checksum CHECKSUM("printing_area_boundary_min_x")
#define printing_area_boundary_min_y_checksum CHECKSUM("printing_area_boundary_min_y")
#define printing_area_boundary_min_z_checksum CHECKSUM("printing_area_boundary_min_z")
// END MODIF boundaries
...
void Robot::on_module_loaded()
{
    ...
    // BEGIN MODIF g790_g791_blocking
    this->register_for_event(ON_GCODE_EXECUTE);
    // END MODIF g790_g791_blocking
    // BEGIN MODIF client_print
...
void Robot::on_config_reload(void *argument)
{
    ...
    //this->clearToolOffset();
    // BEGIN MODIF boundaries
    max_coord_value[X_AXIS] = THEKERNEL->config->value(printing_area_boundary_max_x_checksum)->by_default(340.0F)->as_number();
    max_coord_value[Y_AXIS] = THEKERNEL->config->value(printing_area_boundary_max_y_checksum)->by_default(200.0F)->as_number();
    max_coord_value[Z_AXIS] = THEKERNEL->config->value(printing_area_boundary_max_z_checksum)->by_default(300.0F)->as_number();
    min_coord_value[X_AXIS] = THEKERNEL->config->value(printing_area_boundary_min_x_checksum)->by_default(20.0F)->as_number();
    min_coord_value[Y_AXIS] = THEKERNEL->config->value(printing_area_boundary_min_y_checksum)->by_default(0.0F)->as_number();
    min_coord_value[Z_AXIS] = THEKERNEL->config->value(printing_area_boundary_min_z_checksum)->by_default(0.0F)->as_number();
    // END MODIF boundaries
}
...
// BEGIN MODIF move_ony_if_axis_position_known
static bool are_axes_position_known(bool x, bool y, bool z) {
	bool* axis_position_known;
	if (PublicData::get_value(endstops_checksum, axis_position_known_checksum, (void**)&axis_position_known)) {
		return (!x||axis_position_known[X_AXIS]) && (!y||axis_position_known[Y_AXIS]) && (!z||axis_position_known[Z_AXIS]);
	} else {
		return false;
	}
}
...
void Robot::on_gcode_received(void *argument)
{
    ...
    if( gcode->has_g) {
        switch( gcode->g ) {
            ...
            // BEGIN MODIF move_ony_if_axis_position_known
            case 1:
                if (gcode->has_letter('W') && gcode->get_int('W')) {
                    // Only move if all axes being moved are known
                	if (!are_axes_position_known(gcode->has_letter('X'), gcode->has_letter('Y'), gcode->has_letter('Z'))) {
                		// Cancel movement!! One of the axes needed is not known.
                		return;
                	}
                }
                this->motion_mode = MOTION_MODE_LINEAR;
            break;
            // END MODIF move_ony_if_axis_position_known
        ...
        switch( gcode->m ) {
            ...
            // BEGIN MODIF print_volume_configurable
            case 808: {	// M808 X<xmax> Y<ymax> Z<zmax> I<xmin> J<ymin> K<zmin>
                if (gcode->has_letter('X'))
                    max_coord_value[X_AXIS] = gcode->get_value('X');
                if (gcode->has_letter('Y'))
                    max_coord_value[Y_AXIS] = gcode->get_value('Y');
                if (gcode->has_letter('Z'))
                    max_coord_value[Z_AXIS] = gcode->get_value('Z');
                if (gcode->has_letter('I'))
                    min_coord_value[X_AXIS] = gcode->get_value('I');
                if (gcode->has_letter('J'))
                    min_coord_value[Y_AXIS] = gcode->get_value('J');
                if (gcode->has_letter('K'))
                    min_coord_value[Z_AXIS] = gcode->get_value('K');
                break;
			}
            // END MODIF print_volume_configurable
        }
    }
    ...
    // BEGIN MODIF boundaries
    // TODO Truncating like this can deform objects being printed outside the 
    // printable area. This is not a real problem, because no one should exceed
    // the printable area. For example, going from X10 Y30 to X-10 Y10 should
    // draw a segment from X10 Y30 to X0 Y20, and from there on, another to X0
    // Y10. This code would draw one segment from X10 Y30 to X0 Y10 instead.
    for (char letter = 'X'; letter <= 'Z'; letter++) {
        if ( gcode->has_letter(letter) ) {
            target[letter - 'X'] = max(min(target[letter - 'X'], max_coord_value[letter - 'X']), min_coord_value[letter - 'X']);
        }
    }
    // END MODIF boundaries
...
// BEGIN MODIF g790_g791_blocking
void Robot::on_gcode_execute(void* argument){
    Gcode* gcode = static_cast<Gcode*>(argument);

    if( gcode->has_g) {
        switch( gcode->g ) {
            case 790: this->absolute_mode = true;break;
            case 791: this->absolute_mode = false;break;
        }
    }
}
// END MODIF g790_g791_blocking
...
void Robot::reset_axis_position(float position, int axis)
{
    ...
    // BEGIN MODIF autolevel
    memcpy(this->transformed_last_milestone, this->last_milestone, sizeof(this->last_milestone));
...
// BEGIN MODIF client_print
void Robot::on_print_status_change(void * args) {
	// Whenever a print is started or finished, reset the speed multiplier
    ...
    if (e.event == pe_begin || e.event == pe_end) {
        ...
        this->seconds_per_minute = 60.0;
    }
}
bool Robot::dimensions_are_valid() {
    return this->max_coord_value[X_AXIS] <= X_UPPER_BOUND &&
        this->max_coord_value[Y_AXIS] <= Y_UPPER_BOUND &&
        this->max_coord_value[Z_AXIS] <= Z_UPPER_BOUND;
};
// END MODIF client_print
