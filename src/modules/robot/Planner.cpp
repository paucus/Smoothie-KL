...
void Planner::append_block( float actuator_pos[], float rate_mm_s, float distance, float unit_vec[] )
{
    ...
    block->steps_event_count_idx = (block->steps[ALPHA_STEPPER] > block->steps[BETA_STEPPER])?X_AXIS:Y_AXIS;
    if (block->steps[block->steps_event_count_idx] < block->steps[GAMMA_STEPPER]){
        block->steps_event_count_idx = GAMMA_STEPPER;
    }
    // END MODIF free_heap
